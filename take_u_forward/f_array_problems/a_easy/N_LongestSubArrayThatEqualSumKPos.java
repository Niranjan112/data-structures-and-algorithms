...
public class N_LongestSubArrayThatEqualSumKPos {
  ...
  public static void main(String[] args) {
    ...
    * */
    ...
    optimalApproach(new int[] {2,3,11,10,1}, 10); // O(N) time complexity and O(1) space complexity
  }
  ...
  public static void betterApproach(int[] arr, int k) {
    // Note: This solution will work for all numbers in array i.e positives, negatives and 0.
  ...
  public static void optimalApproach(int[] arr, int k) {
    // Note: This solution will only work if numbers in array are positives and 0.

    int left = 0, right = 0;
    int maxSubArrayLength = 0;
    long sum = arr[0]; //  Take first element as the sum.

    while (right < arr.length) {

      // If left is less or equal to right and sum is greater than k.
      // Then keep subtracting the number from left until sum become less than k.
      while(left <= right && sum > k) {
        sum -= arr[left];
        left++;
      }

      // If sum becomes equals to k then, compute the max sub array.
      if(sum == k ) {
        maxSubArrayLength = Math.max(maxSubArrayLength, right - left + 1);
      }

      // Keep moving forward the right and adding the number to the sum until right is not move out of array.
      right++;
      if(right < arr.length) {
        sum += arr[right];
      }
